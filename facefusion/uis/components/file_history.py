...
def update_galleries() -> None:
    ...
    try:
        ...
        print(f"Error updating galleries: {str(e)}")
...
def load_source_file(selected_index: int) -> None:
    ...
    if 0 <= selected_index < len(source_files):
        ...
        if is_image(selected_file["path"]):
            ...
            # Update UI
            from facefusion.uis.components.source import SOURCE_FILE
            if SOURCE_FILE:
                SOURCE_FILE.update(value=[{"name": selected_file["path"], "path": selected_file["path"]}])
...
def load_target_file(selected_index: int) -> None:
    ...
    if 0 <= selected_index < len(target_files):
        ...
        if TARGET_FILE:
            TARGET_FILE.update(value={"name": selected_file["path"], "path": selected_file["path"]})


def clear_history() -> Tuple[gradio.Gallery, gradio.Gallery]:
    """Clear file history"""
    file_history.clear_history()
    # Update the galleries with empty values
    return SOURCE_FILES_GALLERY.update(value=[]), TARGET_FILES_GALLERY.update(value=[])
